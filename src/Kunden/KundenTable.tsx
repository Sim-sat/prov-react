...
export function KundenTable() {
    ...
    const rows = sortedData.map((row) => (
        <Table.Tr onDoubleClick={() => navigate(`/customer/view/${row.cud}`)} key={row.cud}>
            <Table.Td>{row.cud}</Table.Td>
            <Table.Td>{row.name}</Table.Td>
            <Table.Td>{row.shortName}</Table.Td>
            <Table.Td>{row.reseller}</Table.Td>
            ...
            <Table highlightOnHover withTableBorder horizontalSpacing="md" verticalSpacing="xs" miw={700}
                ...
                <Table.Tbody>
                    <Table.Tr>
                        ...
                        <Th
                            sorted={sortBy === 'reseller'}
                            reversed={reverseSortDirection}
                            onSort={() => setSorting('reseller')}
                        >
                            Reseller
                        </Th>
